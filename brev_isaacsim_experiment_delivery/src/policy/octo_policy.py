"""Octo-style synthetic policy backed by a learned linear checkpoint."""
...
class Octo(BasePolicy):
    def load_model(self) -> None:
        ...
        if path.exists():
            ...
            self.model["noise_scale"] = np.float32(0.065)
    ...
    def act(self, observation: Dict[str, Any]) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        ensemble_size = int(self.config.get("ensemble_size", 5))

        actions = []
        confidences = []
        for _ in range(max(ensemble_size, 1)):
            member_weights = weights + self.rng.normal(0.0, noise_scale * 0.6, size=weights.shape)
            member_bias = bias + self.rng.normal(0.0, noise_scale * 0.4, size=bias.shape)
            latent = member_weights @ x + member_bias
            action = np.tanh(latent).astype(np.float32)
            conf_logit = float(1.2 * (conf_weights @ x + conf_bias) + 0.35 - 0.22 * np.var(action) + self.rng.normal(0.0, noise_scale * 0.8))
